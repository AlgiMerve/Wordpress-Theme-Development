...
document.getElementById("btn_select").addEventListener("click", function () {
  ...
  if (ic_eleman_sayisi != 0) {
    ...
    for (let i = 0; i < ic_eleman_sayisi; i++) {
      var tr = `<tr><th scope="row">${dizi_sira[i]}</th><td>${dizi_tc[i]}</td><td>${dizi_ad[i]}</td><td><input class="inp" disabled style="text-align: center" type="text" name="ogr_okh_${i}" id="ogr_okh_${i}"></td><td><input class="inp_tds" disabled style="text-align: center" type="text" name="ogr_tds_${i}" id="ogr_tds_${i}"></td></tr>`;
      tbody.innerHTML += tr;
    }
...
function SetInf(){
  document.getElementById("modal_body_table").children[drcl].children[3].children[0].value = document.getElementById("readspead").textContent
  document.getElementById("modal_body_table").children[drcl].children[4].children[0].value = soru_dogru_sayisi*20;
  drcl++;
}
