...
document.getElementById("btn_select").addEventListener("click", function () {
  ...
  if (ic_eleman_sayisi != 0) {
    ...
    for (let i = 0; i < ic_eleman_sayisi; i++) {
      var tr = `<tr><th scope="row">${dizi_sira[i]}</th><td>${dizi_tc[i]}</td><td>${dizi_ad[i]}</td><td><input class="inp" disabled style="text-align: center" type="text" name="ogr_sure_${i}" id="ogr_sure_${i}"></td></tr>`;
      tbody.innerHTML += tr;
    }
...
//Modal Kısmı Kaç Tane İnput olacağının belirlendiği kısım

function SetInf() {
  document.getElementById("modal_body_table").children[
    drcl
  ].children[3].children[0].value = timeV;
  drcl++;
}
